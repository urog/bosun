...
func (s *Schedule) Host(filter string) (map[string]*HostData, error) {
	...
	silences := s.Silenced()
	// TODO Grap timestamps on these tagsets so I don't show stuff that is too old
	...
	for name, host := range hosts {
		...
		for _, ts := range icmpTimeOutTags {
			...
			source, ok := ts["host"]
			if !ok {
				slog.Errorf("couldn't find source tag for icmp data for host", host.Name)
			}
		...
		for _, ts := range hwPowerSuppliesTags {
			...
			id, ok := ts["id"]
			if !ok {
				return nil, fmt.Errorf("couldn't find power supply tag for host", host.Name)
			}
		...
		for _, ts := range hwPhysicalDiskTags {
			...
			id, ok := ts["id"]
			if !ok {
				return nil, fmt.Errorf("couldn't find physical disk id tag for host", host.Name)
			}
		...
		for _, ts := range hwVirtualDiskTags {
			...
			id, ok := ts["id"]
			if !ok {
				return nil, fmt.Errorf("couldn't find virtual disk id tag for host", host.Name)
			}
		...
		for _, ts := range hwControllersTags {
			...
			id, ok := ts["id"]
			if !ok {
				return nil, fmt.Errorf("couldn't find controller id tag for host", host.Name)
			}
			...
			for _, m := range hostMetadata {
				...
				case string:
					...
					case "controllerMeta":
						...
						if err != nil {
							slog.Errorf("error unmarshalling controller meta for host %s", host.Name, err)
						} else {
							host.Hardware.Storage.Controllers[id] = c
						}
					}
		...
		for _, ts := range diskTags {
			...
			disk, ok := ts["disk"]
			if !ok {
				return nil, fmt.Errorf("couldn't find disk tag for host", host.Name)
			}
			// Should probably update this to match the memory patter,
			// only update if timestamp is new and no error
		...
		// Source Issue? (1) Some things are created based on the existance of it in .Last,
		// other things are created off the existance of metadata
		// Process Metadata, and get some stats keyed off of metadata instead of
		// Search's Last
		for _, m := range hostMetadata {
			if m.Time.Before(time.Now().Add(-timeFilterAge)) {
				continue
			...
			case string:
				switch m.Name {
				// TODO I'm keeping this commented out until the all collectors are sending JSON
				// arrays and not addr tags
				// case "addr":
				// 	if iface != nil {
